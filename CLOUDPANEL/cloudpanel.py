...
@client.tree.command(name='top-features', description="Returns the top 10 most wanted feature requests")
@app_commands.choices(results=[
    ...
    app_commands.Choice(name="7", value="8"),
    app_commands.Choice(name="8", value="8")])
async def top_features(interaction: discord.Interaction, results: Optional[app_commands.Choice[int]]):
    embed = discord.Embed(title="Top 10 Most-wanted Features for CloudPanel", color=discord.Color.dark_teal())
    url = f"https://feature-requests.cloudpanel.io/api/v1/posts?view=most-wanted&limit={results}&key={API_KEY}"
    json_url = requests.get(url)
    data = json.loads(json_url.text)
    for d in data:
        embed.add_field(name=d["title"], value=f"https://feature-requests.cloudpanel.io/posts/{d['id']}/{d['slug']}", inline=False)
        embed.add_field(name="Author", value=d['user']['name'])
        embed.add_field(name="Votes", value=d['votesCount'])
...
TOKEN = "MTA0MjQyMTM0MTQ4OTQ2MzMwNw.Ge0lCk.MuH10eB-R0_fbUPwZAANsRoW1xNSYhKOlPIRLs"
